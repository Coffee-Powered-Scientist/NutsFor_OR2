...
Leaching_LNS_Cumulative<-Leaching_LNS %>% subset(group_id %in% 8 & YEAR %in% Years) %>%
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Rot="Cumulative", Loss1="Leaching",Loss2="Background Leaching" ,Site="LNS", Harvest="80WTH", N=NO3+NH4)  %>% 
  select(N, P, K, Ca, Site, Harvest,Rot, Loss1, Loss2)


Leaching_LNS_Excess<-Ex_Table %>% subset(Harvest %in% "80WTH" & Site %in% "LNS")%>% 
...
Leaching_LNS_Background_R1<-Leaching_LNS %>% subset(group_id %in% 8 & YEAR %in% Years_R1) %>% 
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Site="LNS", Harvest="80WTH",Rot="Rotation 1", Loss1="Leaching",Loss2="Background Leaching", N=NO3+NH4) %>%
...
Leaching_LNS_Excess_R1<-Ex_Table_R1 %>% rename(Ca=Ca_Ex_R1, K=K_Ex_R1, N=N_Ex_R1, P=P_Ex_R1) %>%
  select(Ca, K, N, P, Site, Harvest) %>% subset(Site %in% "LNS" & Harvest %in% "80WTH") %>%
...
Net_Removal_LNS_R1<-Tree_Nut_LNS %>% subset(YEAR %in% c(2065)) %>% subset(Month %in% 8)  %>% mutate(Site="LNS", Harvest="80WTH", Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")

Net_Removal_LNS_R1$P_Brh<-as.numeric(Net_Removal_LNS_R1$P_Brh)

Net_Removal_LNS_R1<-Net_Removal_LNS_R1 %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="LNS", Harvest="80WTH",Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")
...
Net_Removal_LNS<-Net_Removal_LNS %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="LNS", Harvest="80WTH",Rot="Cumulative", Loss1="Harvest", Loss2="Harvest")
...
AEC_All_LNS_R1<-AEC_All_LNS %>% subset(Date %in% c('2005-08-01', '2065-08-01')) %>% 
...
AEC_All_LNS_R1<-AEC_All_LNS_R1 %>% subset(Date %in% c('2005-08-01')) %>% rename(P=PO4_Net) %>% 
  mutate(N=0, K=0, Ca=0, Site="LNS", Harvest="80WTH") %>% 
  select(N, P, K, Ca,Site, Harvest, Rot, Loss1, Loss2)
...
Ca_LNS<-ggplot(subset(LNS_Melt, variable %in% c("Ca") & Loss2 != c("Adsorption")), aes(x=Rot, y=value, fill=Loss2))+
  ...
  theme_classic2()+theme(plot.title = element_text(hjust = 0.5), legend.key.size = unit(1.5, 'cm'),
...
Leaching_HNS_Cumulative<-Leaching_HNS %>% subset(group_id %in% 8 & YEAR %in% Years) %>%
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Rot="Cumulative", Loss1="Leaching",Loss2="Background Leaching" ,Site="HNS", Harvest="80WTH", N=NO3+NH4)  %>% 
  select(N, P, K, Ca, Site, Harvest,Rot, Loss1, Loss2)


Leaching_HNS_Excess<-Ex_Table %>% subset(Harvest %in% "80WTH" & Site %in% "HNS")%>% 
  mutate(Rot="Cumulative", Loss1="Leaching",Loss2="Post-Harvest Leaching")%>% 
  rename(Ca=Ca_Ex, Mg=Mg_Ex, K=K_Ex, N=N_Ex, P=P_Ex, S=SO4_Ex) %>%
  select(Ca, K, N, P, Site, Harvest,Rot, Loss1, Loss2)
...
Leaching_HNS_Background_R1<-Leaching_HNS %>% subset(group_id %in% 8 & YEAR %in% Years_R1) %>% 
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Site="HNS", Harvest="80WTH",Rot="Rotation 1", Loss1="Leaching",Loss2="Background Leaching", N=NO3+NH4) %>%
  select(N, P, K, Ca, Rot, Loss1, Loss2, Site, Harvest)

Leaching_HNS_Excess_R1<-Ex_Table_R1 %>% rename(Ca=Ca_Ex_R1, K=K_Ex_R1, N=N_Ex_R1, P=P_Ex_R1) %>%
  select(Ca, K, N, P, Site, Harvest) %>% subset(Site %in% "HNS" & Harvest %in% "80WTH") %>%
  mutate(Rot="Rotation 1", Loss1="Leaching", Loss2="Post-Harvest Leaching")
...
Net_Removal_HNS_R1<-Tree_Nut_HNS %>% subset(YEAR %in% c(2065)) %>% subset(Month %in% 8)  %>% mutate(Site="HNS", Harvest="80WTH", Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")

Net_Removal_HNS_R1$P_Brh<-as.numeric(Net_Removal_HNS_R1$P_Brh)

Net_Removal_HNS_R1<-Net_Removal_HNS_R1 %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="HNS", Harvest="80WTH",Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")
...
Net_Removal_HNS<-Net_Removal_HNS %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="HNS", Harvest="80WTH",Rot="Cumulative", Loss1="Harvest", Loss2="Harvest")
...
Leaching_LNB_Cumulative<-Leaching_LNB %>% subset(group_id %in% 8 & YEAR %in% Years) %>%
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Rot="Cumulative", Loss1="Leaching",Loss2="Background Leaching" ,Site="LNB", Harvest="80WTH", N=NO3+NH4)  %>% 
  select(N, P, K, Ca, Site, Harvest,Rot, Loss1, Loss2)


Leaching_LNB_Excess<-Ex_Table %>% subset(Harvest %in% "80WTH" & Site %in% "LNB")%>% 
  mutate(Rot="Cumulative", Loss1="Leaching",Loss2="Post-Harvest Leaching")%>% 
  rename(Ca=Ca_Ex, Mg=Mg_Ex, K=K_Ex, N=N_Ex, P=P_Ex, S=SO4_Ex) %>%
  select(Ca, K, N, P, Site, Harvest,Rot, Loss1, Loss2)
...
Leaching_LNB_Background_R1<-Leaching_LNB %>% subset(group_id %in% 8 & YEAR %in% Years_R1) %>% 
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Site="LNB", Harvest="80WTH",Rot="Rotation 1", Loss1="Leaching",Loss2="Background Leaching", N=NO3+NH4) %>%
  select(N, P, K, Ca, Rot, Loss1, Loss2, Site, Harvest)

Leaching_LNB_Excess_R1<-Ex_Table_R1 %>% rename(Ca=Ca_Ex_R1, K=K_Ex_R1, N=N_Ex_R1, P=P_Ex_R1) %>%
  select(Ca, K, N, P, Site, Harvest) %>% subset(Site %in% "LNB" & Harvest %in% "80WTH") %>%
  mutate(Rot="Rotation 1", Loss1="Leaching", Loss2="Post-Harvest Leaching")
...
Net_Removal_LNB_R1<-Tree_Nut_LNB %>% subset(YEAR %in% c(2065)) %>% subset(Month %in% 8)  %>% mutate(Site="LNB", Harvest="80WTH", Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")

Net_Removal_LNB_R1$P_Brh<-as.numeric(Net_Removal_LNB_R1$P_Brh)

Net_Removal_LNB_R1<-Net_Removal_LNB_R1 %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="LNB", Harvest="80WTH",Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")
...
Net_Removal_LNB<-Net_Removal_LNB %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="LNB", Harvest="80WTH",Rot="Cumulative", Loss1="Harvest", Loss2="Harvest")
...
Leaching_HNB_Cumulative<-Leaching_HNB %>% subset(group_id %in% 8 & YEAR %in% Years) %>%
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Rot="Cumulative", Loss1="Leaching",Loss2="Background Leaching" ,Site="HNB", Harvest="80WTH", N=NO3+NH4)  %>% 
  select(N, P, K, Ca, Site, Harvest,Rot, Loss1, Loss2)


Leaching_HNB_Excess<-Ex_Table %>% subset(Harvest %in% "80WTH" & Site %in% "HNB")%>% 
  mutate(Rot="Cumulative", Loss1="Leaching",Loss2="Post-Harvest Leaching")%>% 
  rename(Ca=Ca_Ex, Mg=Mg_Ex, K=K_Ex, N=N_Ex, P=P_Ex, S=SO4_Ex) %>%
  select(Ca, K, N, P, Site, Harvest,Rot, Loss1, Loss2)
...
Leaching_HNB_Background_R1<-Leaching_HNB %>% subset(group_id %in% 8 & YEAR %in% Years_R1) %>% 
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Site="HNB", Harvest="80WTH",Rot="Rotation 1", Loss1="Leaching",Loss2="Background Leaching", N=NO3+NH4) %>%
  select(N, P, K, Ca, Rot, Loss1, Loss2, Site, Harvest)

Leaching_HNB_Excess_R1<-Ex_Table_R1 %>% rename(Ca=Ca_Ex_R1, K=K_Ex_R1, N=N_Ex_R1, P=P_Ex_R1) %>%
  select(Ca, K, N, P, Site, Harvest) %>% subset(Site %in% "HNB" & Harvest %in% "80WTH") %>%
  mutate(Rot="Rotation 1", Loss1="Leaching", Loss2="Post-Harvest Leaching")
...
Net_Removal_HNB_R1<-Tree_Nut_HNB %>% subset(YEAR %in% c(2065)) %>% subset(Month %in% 8)  %>% mutate(Site="HNB", Harvest="80WTH", Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")

Net_Removal_HNB_R1$P_Brh<-as.numeric(Net_Removal_HNB_R1$P_Brh)

Net_Removal_HNB_R1<-Net_Removal_HNB_R1 %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="HNB", Harvest="80WTH",Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")
...
Net_Removal_HNB<-Net_Removal_HNB %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="HNB", Harvest="80WTH",Rot="Cumulative", Loss1="Harvest", Loss2="Harvest")
