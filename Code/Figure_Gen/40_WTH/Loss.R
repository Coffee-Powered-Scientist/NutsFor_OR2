...
Leaching_LNS<-read.csv2("~/Project_Master/Test_Rep/Output/Manuscript/LN_SED/40_WTH/Edited Data/Soil_Solution_All2.csv")

# New Method

Leaching_LNS_Cumulative<-Leaching_LNS %>% subset(group_id %in% 8 & YEAR %in% Years) %>%
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Rot="Cumulative", Loss1="Leaching",Loss2="Background Leaching" ,Site="LNS", Harvest="40WTH", N=NO3+NH4)  %>% 
  select(N, P, K, Ca, Site, Harvest,Rot, Loss1, Loss2)


Leaching_LNS_Excess<-Ex_Table %>% subset(Harvest %in% "40WTH" & Site %in% "LNS")%>% 
...
Leaching_LNS_Background_R1<-Leaching_LNS %>% subset(group_id %in% 8 & YEAR %in% Years_R1) %>% 
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Site="LNS", Harvest="40WTH",Rot="Rotation 1", Loss1="Leaching",Loss2="Background Leaching", N=NO3+NH4) %>%
...
Leaching_LNS_Excess_R1<-Ex_Table_R1 %>% rename(Ca=Ca_Ex_R1, K=K_Ex_R1, N=N_Ex_R1, P=P_Ex_R1) %>%
  select(Ca, K, N, P, Site, Harvest) %>% subset(Site %in% "LNS" & Harvest %in% "40WTH") %>%
  mutate(Rot="Rotation 1", Loss1="Leaching", Loss2="Post-Harvest Leaching")



Tree_Nut_LNS<-read.csv2("~/Project_Master/Test_Rep/Output/Manuscript/LN_SED/40_WTH/Edited Data/Tree_Nut_All.csv")

Net_Removal_LNS_R1<-Tree_Nut_LNS %>% subset(YEAR %in% c(2025)) %>% subset(Month %in% 8)  %>% mutate(Site="LNS", Harvest="40WTH", Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")

Net_Removal_LNS_R1$P_Brh<-as.numeric(Net_Removal_LNS_R1$P_Brh)

Net_Removal_LNS_R1<-Net_Removal_LNS_R1 %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="LNS", Harvest="40WTH",Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")
...
Net_Removal_LNS<-Net_Removal_LNS %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="LNS", Harvest="40WTH",Rot="Cumulative", Loss1="Harvest", Loss2="Harvest")

Net_Removal_LNS_R1$Ca<-Net_Removal_LNS_R1$Ca_F*.99+Net_Removal_LNS_R1$Ca_Bol*.97*.99+Net_Removal_LNS_R1$Ca_Brh*.99+Net_Removal_LNS_R1$Ca_Brk*.9*.99

Net_Removal_LNS_R1$N<-Net_Removal_LNS_R1$N_F*.99+Net_Removal_LNS_R1$N_Bol*.97*.99+Net_Removal_LNS_R1$N_Brh*.99+Net_Removal_LNS_R1$N_Brk*.9*.99

Net_Removal_LNS_R1$K<-Net_Removal_LNS_R1$K_F*.99+Net_Removal_LNS_R1$K_Bol*.97*.99+Net_Removal_LNS_R1$K_Brh*.99+Net_Removal_LNS_R1$K_Brk*.9*.99

Net_Removal_LNS_R1$P<-Net_Removal_LNS_R1$P_F*.99+Net_Removal_LNS_R1$P_Bol*.97*.99+Net_Removal_LNS_R1$P_Brh*.99+Net_Removal_LNS_R1$P_Brk*.9*.99

Net_Removal_LNS$Ca<-Net_Removal_LNS$Ca_F*.99+Net_Removal_LNS$Ca_Bol*.97*.99+Net_Removal_LNS$Ca_Brh*.99+Net_Removal_LNS$Ca_Brk*.9*.99

Net_Removal_LNS$N<-Net_Removal_LNS$N_F*.99+Net_Removal_LNS$N_Bol*.97*.99+Net_Removal_LNS$N_Brh*.99+Net_Removal_LNS$N_Brk*.9*.99

Net_Removal_LNS$K<-Net_Removal_LNS$K_F*.99+Net_Removal_LNS$K_Bol*.97*.99+Net_Removal_LNS$K_Brh*.99+Net_Removal_LNS$K_Brk*.9*.99

Net_Removal_LNS$P<-Net_Removal_LNS$P_F*.99+Net_Removal_LNS$P_Bol*.97*.99+Net_Removal_LNS$P_Brh*.99+Net_Removal_LNS$P_Brk*.9*.99
...
AEC_All_LNS<-read.csv2("~/Project_Master/Test_Rep/Output/Manuscript/LN_SED/40_WTH/Edited Data/AEC_All.csv")
...
AEC_All_LNS_R1<-AEC_All_LNS_R1 %>% subset(Date %in% c('2005-08-01')) %>% rename(P=PO4_Net) %>% 
  mutate(N=0, K=0, Ca=0, Site="LNS", Harvest="40WTH") %>% 
  select(N, P, K, Ca,Site, Harvest, Rot, Loss1, Loss2)
...
Leaching_HNS<-read.csv2("~/Project_Master/Test_Rep/Output/Manuscript/HN_SED/40_WTH/Edited Data/Soil_Solution_All2.csv")

# New Method

Leaching_HNS_Cumulative<-Leaching_HNS %>% subset(group_id %in% 8 & YEAR %in% Years) %>%
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Rot="Cumulative", Loss1="Leaching",Loss2="Background Leaching" ,Site="HNS", Harvest="40WTH", N=NO3+NH4)  %>% 
  select(N, P, K, Ca, Site, Harvest,Rot, Loss1, Loss2)


Leaching_HNS_Excess<-Ex_Table %>% subset(Harvest %in% "40WTH" & Site %in% "HNS")%>% 
  mutate(Rot="Cumulative", Loss1="Leaching",Loss2="Post-Harvest Leaching")%>% 
  rename(Ca=Ca_Ex, Mg=Mg_Ex, K=K_Ex, N=N_Ex, P=P_Ex, S=SO4_Ex) %>%
  select(Ca, K, N, P, Site, Harvest,Rot, Loss1, Loss2)
...
Leaching_HNS_Background_R1<-Leaching_HNS %>% subset(group_id %in% 8 & YEAR %in% Years_R1) %>% 
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Site="HNS", Harvest="40WTH",Rot="Rotation 1", Loss1="Leaching",Loss2="Background Leaching", N=NO3+NH4) %>%
  select(N, P, K, Ca, Rot, Loss1, Loss2, Site, Harvest)

Leaching_HNS_Excess_R1<-Ex_Table_R1 %>% rename(Ca=Ca_Ex_R1, K=K_Ex_R1, N=N_Ex_R1, P=P_Ex_R1) %>%
  select(Ca, K, N, P, Site, Harvest) %>% subset(Site %in% "HNS" & Harvest %in% "40WTH") %>%
  mutate(Rot="Rotation 1", Loss1="Leaching", Loss2="Post-Harvest Leaching")



Tree_Nut_HNS<-read.csv2("~/Project_Master/Test_Rep/Output/Manuscript/HN_SED/40_WTH/Edited Data/Tree_Nut_All.csv")

Net_Removal_HNS_R1<-Tree_Nut_HNS %>% subset(YEAR %in% c(2025)) %>% subset(Month %in% 8)  %>% mutate(Site="HNS", Harvest="40WTH", Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")

Net_Removal_HNS_R1$P_Brh<-as.numeric(Net_Removal_HNS_R1$P_Brh)

Net_Removal_HNS_R1<-Net_Removal_HNS_R1 %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="HNS", Harvest="40WTH",Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")
...
Net_Removal_HNS<-Net_Removal_HNS %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="HNS", Harvest="40WTH",Rot="Cumulative", Loss1="Harvest", Loss2="Harvest")

Net_Removal_HNS_R1$Ca<-Net_Removal_HNS_R1$Ca_F*.99+Net_Removal_HNS_R1$Ca_Bol*.97*.99+Net_Removal_HNS_R1$Ca_Brh*.99+Net_Removal_HNS_R1$Ca_Brk*.9*.99

Net_Removal_HNS_R1$N<-Net_Removal_HNS_R1$N_F*.99+Net_Removal_HNS_R1$N_Bol*.97*.99+Net_Removal_HNS_R1$N_Brh*.99+Net_Removal_HNS_R1$N_Brk*.9*.99

Net_Removal_HNS_R1$K<-Net_Removal_HNS_R1$K_F*.99+Net_Removal_HNS_R1$K_Bol*.97*.99+Net_Removal_HNS_R1$K_Brh*.99+Net_Removal_HNS_R1$K_Brk*.9*.99

Net_Removal_HNS_R1$P<-Net_Removal_HNS_R1$P_F*.99+Net_Removal_HNS_R1$P_Bol*.97*.99+Net_Removal_HNS_R1$P_Brh*.99+Net_Removal_HNS_R1$P_Brk*.9*.99

Net_Removal_HNS$Ca<-Net_Removal_HNS$Ca_F*.99+Net_Removal_HNS$Ca_Bol*.97*.99+Net_Removal_HNS$Ca_Brh*.99+Net_Removal_HNS$Ca_Brk*.9*.99

Net_Removal_HNS$N<-Net_Removal_HNS$N_F*.99+Net_Removal_HNS$N_Bol*.97*.99+Net_Removal_HNS$N_Brh*.99+Net_Removal_HNS$N_Brk*.9*.99

Net_Removal_HNS$K<-Net_Removal_HNS$K_F*.99+Net_Removal_HNS$K_Bol*.97*.99+Net_Removal_HNS$K_Brh*.99+Net_Removal_HNS$K_Brk*.9*.99

Net_Removal_HNS$P<-Net_Removal_HNS$P_F*.99+Net_Removal_HNS$P_Bol*.97*.99+Net_Removal_HNS$P_Brh*.99+Net_Removal_HNS$P_Brk*.9*.99
...
AEC_All_HNS<-read.csv2("~/Project_Master/Test_Rep/Output/Manuscript/HN_SED/40_WTH/Edited Data/AEC_All.csv")
...
AEC_All_HNS_R1<-AEC_All_HNS_R1 %>% subset(Date %in% c('2005-08-01')) %>% rename(P=PO4_Net) %>% 
  mutate(N=0, K=0, Ca=0, Site="HNS", Harvest="40WTH") %>% 
  select(N, P, K, Ca,Site, Harvest, Rot, Loss1, Loss2)
...
Leaching_LNB<-read.csv2("~/Project_Master/Test_Rep/Output/Manuscript/LN_BAS/40_WTH/Edited Data/Soil_Solution_All2.csv")

# New Method

Leaching_LNB_Cumulative<-Leaching_LNB %>% subset(group_id %in% 8 & YEAR %in% Years) %>%
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Rot="Cumulative", Loss1="Leaching",Loss2="Background Leaching" ,Site="LNB", Harvest="40WTH", N=NO3+NH4)  %>% 
  select(N, P, K, Ca, Site, Harvest,Rot, Loss1, Loss2)


Leaching_LNB_Excess<-Ex_Table %>% subset(Harvest %in% "40WTH" & Site %in% "LNB")%>% 
  mutate(Rot="Cumulative", Loss1="Leaching",Loss2="Post-Harvest Leaching")%>% 
  rename(Ca=Ca_Ex, Mg=Mg_Ex, K=K_Ex, N=N_Ex, P=P_Ex, S=SO4_Ex) %>%
  select(Ca, K, N, P, Site, Harvest,Rot, Loss1, Loss2)
...
Leaching_LNB_Background_R1<-Leaching_LNB %>% subset(group_id %in% 8 & YEAR %in% Years_R1) %>% 
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Site="LNB", Harvest="40WTH",Rot="Rotation 1", Loss1="Leaching",Loss2="Background Leaching", N=NO3+NH4) %>%
  select(N, P, K, Ca, Rot, Loss1, Loss2, Site, Harvest)

Leaching_LNB_Excess_R1<-Ex_Table_R1 %>% rename(Ca=Ca_Ex_R1, K=K_Ex_R1, N=N_Ex_R1, P=P_Ex_R1) %>%
  select(Ca, K, N, P, Site, Harvest) %>% subset(Site %in% "LNB" & Harvest %in% "40WTH") %>%
  mutate(Rot="Rotation 1", Loss1="Leaching", Loss2="Post-Harvest Leaching")



Tree_Nut_LNB<-read.csv2("~/Project_Master/Test_Rep/Output/Manuscript/LN_BAS/40_WTH/Edited Data/Tree_Nut_All.csv")

Net_Removal_LNB_R1<-Tree_Nut_LNB %>% subset(YEAR %in% c(2025)) %>% subset(Month %in% 8)  %>% mutate(Site="LNB", Harvest="40WTH", Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")

Net_Removal_LNB_R1$P_Brh<-as.numeric(Net_Removal_LNB_R1$P_Brh)

Net_Removal_LNB_R1<-Net_Removal_LNB_R1 %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="LNB", Harvest="40WTH",Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")
...
Net_Removal_LNB<-Net_Removal_LNB %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="LNB", Harvest="40WTH",Rot="Cumulative", Loss1="Harvest", Loss2="Harvest")

Net_Removal_LNB_R1$Ca<-Net_Removal_LNB_R1$Ca_F*.99+Net_Removal_LNB_R1$Ca_Bol*.97*.99+Net_Removal_LNB_R1$Ca_Brh*.99+Net_Removal_LNB_R1$Ca_Brk*.9*.99

Net_Removal_LNB_R1$N<-Net_Removal_LNB_R1$N_F*.99+Net_Removal_LNB_R1$N_Bol*.97*.99+Net_Removal_LNB_R1$N_Brh*.99+Net_Removal_LNB_R1$N_Brk*.9*.99

Net_Removal_LNB_R1$K<-Net_Removal_LNB_R1$K_F*.99+Net_Removal_LNB_R1$K_Bol*.97*.99+Net_Removal_LNB_R1$K_Brh*.99+Net_Removal_LNB_R1$K_Brk*.9*.99

Net_Removal_LNB_R1$P<-Net_Removal_LNB_R1$P_F*.99+Net_Removal_LNB_R1$P_Bol*.97*.99+Net_Removal_LNB_R1$P_Brh*.99+Net_Removal_LNB_R1$P_Brk*.9*.99

Net_Removal_LNB$Ca<-Net_Removal_LNB$Ca_F*.99+Net_Removal_LNB$Ca_Bol*.97*.99+Net_Removal_LNB$Ca_Brh*.99+Net_Removal_LNB$Ca_Brk*.9*.99

Net_Removal_LNB$N<-Net_Removal_LNB$N_F*.99+Net_Removal_LNB$N_Bol*.97*.99+Net_Removal_LNB$N_Brh*.99+Net_Removal_LNB$N_Brk*.9*.99

Net_Removal_LNB$K<-Net_Removal_LNB$K_F*.99+Net_Removal_LNB$K_Bol*.97*.99+Net_Removal_LNB$K_Brh*.99+Net_Removal_LNB$K_Brk*.9*.99

Net_Removal_LNB$P<-Net_Removal_LNB$P_F*.99+Net_Removal_LNB$P_Bol*.97*.99+Net_Removal_LNB$P_Brh*.99+Net_Removal_LNB$P_Brk*.9*.99
...
AEC_All_LNB<-read.csv2("~/Project_Master/Test_Rep/Output/Manuscript/LN_BAS/40_WTH/Edited Data/AEC_All.csv")
...
AEC_All_LNB_R1<-AEC_All_LNB_R1 %>% subset(Date %in% c('2005-08-01')) %>% rename(P=PO4_Net) %>% 
  mutate(N=0, K=0, Ca=0, Site="LNB", Harvest="40WTH") %>% 
  select(N, P, K, Ca,Site, Harvest, Rot, Loss1, Loss2)
...
Leaching_HNB<-read.csv2("~/Project_Master/Test_Rep/Output/Manuscript/HN_BAS/40_WTH/Edited Data/Soil_Solution_All2.csv")

# New Method

Leaching_HNB_Cumulative<-Leaching_HNB %>% subset(group_id %in% 8 & YEAR %in% Years) %>%
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Rot="Cumulative", Loss1="Leaching",Loss2="Background Leaching" ,Site="HNB", Harvest="40WTH", N=NO3+NH4)  %>% 
  select(N, P, K, Ca, Site, Harvest,Rot, Loss1, Loss2)


Leaching_HNB_Excess<-Ex_Table %>% subset(Harvest %in% "40WTH" & Site %in% "HNB")%>% 
  mutate(Rot="Cumulative", Loss1="Leaching",Loss2="Post-Harvest Leaching")%>% 
  rename(Ca=Ca_Ex, Mg=Mg_Ex, K=K_Ex, N=N_Ex, P=P_Ex, S=SO4_Ex) %>%
  select(Ca, K, N, P, Site, Harvest,Rot, Loss1, Loss2)
...
Leaching_HNB_Background_R1<-Leaching_HNB %>% subset(group_id %in% 8 & YEAR %in% Years_R1) %>% 
  summarise(across(Ca:Si, ~sum(.x, na.rm=TRUE)))  %>% mutate(Site="HNB", Harvest="40WTH",Rot="Rotation 1", Loss1="Leaching",Loss2="Background Leaching", N=NO3+NH4) %>%
  select(N, P, K, Ca, Rot, Loss1, Loss2, Site, Harvest)

Leaching_HNB_Excess_R1<-Ex_Table_R1 %>% rename(Ca=Ca_Ex_R1, K=K_Ex_R1, N=N_Ex_R1, P=P_Ex_R1) %>%
  select(Ca, K, N, P, Site, Harvest) %>% subset(Site %in% "HNB" & Harvest %in% "40WTH") %>%
  mutate(Rot="Rotation 1", Loss1="Leaching", Loss2="Post-Harvest Leaching")



Tree_Nut_HNB<-read.csv2("~/Project_Master/Test_Rep/Output/Manuscript/HN_BAS/40_WTH/Edited Data/Tree_Nut_All.csv")

Net_Removal_HNB_R1<-Tree_Nut_HNB %>% subset(YEAR %in% c(2025)) %>% subset(Month %in% 8)  %>% mutate(Site="HNB", Harvest="40WTH", Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")

Net_Removal_HNB_R1$P_Brh<-as.numeric(Net_Removal_HNB_R1$P_Brh)

Net_Removal_HNB_R1<-Net_Removal_HNB_R1 %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="HNB", Harvest="40WTH",Rot="Rotation 1", Loss1="Harvest", Loss2="Harvest")
...
Net_Removal_HNB<-Net_Removal_HNB %>% summarise(across(DW_F:P_Bol, ~sum(.x, na.rm=TRUE))) %>% mutate(Site="HNB", Harvest="40WTH",Rot="Cumulative", Loss1="Harvest", Loss2="Harvest")

Net_Removal_HNB_R1$Ca<-Net_Removal_HNB_R1$Ca_F*.99+Net_Removal_HNB_R1$Ca_Bol*.97*.99+Net_Removal_HNB_R1$Ca_Brh*.99+Net_Removal_HNB_R1$Ca_Brk*.9*.99

Net_Removal_HNB_R1$N<-Net_Removal_HNB_R1$N_F*.99+Net_Removal_HNB_R1$N_Bol*.97*.99+Net_Removal_HNB_R1$N_Brh*.99+Net_Removal_HNB_R1$N_Brk*.9*.99

Net_Removal_HNB_R1$K<-Net_Removal_HNB_R1$K_F*.99+Net_Removal_HNB_R1$K_Bol*.97*.99+Net_Removal_HNB_R1$K_Brh*.99+Net_Removal_HNB_R1$K_Brk*.9*.99

Net_Removal_HNB_R1$P<-Net_Removal_HNB_R1$P_F*.99+Net_Removal_HNB_R1$P_Bol*.97*.99+Net_Removal_HNB_R1$P_Brh*.99+Net_Removal_HNB_R1$P_Brk*.9*.99

Net_Removal_HNB$Ca<-Net_Removal_HNB$Ca_F*.99+Net_Removal_HNB$Ca_Bol*.97*.99+Net_Removal_HNB$Ca_Brh*.99+Net_Removal_HNB$Ca_Brk*.9*.99

Net_Removal_HNB$N<-Net_Removal_HNB$N_F*.99+Net_Removal_HNB$N_Bol*.97*.99+Net_Removal_HNB$N_Brh*.99+Net_Removal_HNB$N_Brk*.9*.99

Net_Removal_HNB$K<-Net_Removal_HNB$K_F*.99+Net_Removal_HNB$K_Bol*.97*.99+Net_Removal_HNB$K_Brh*.99+Net_Removal_HNB$K_Brk*.9*.99

Net_Removal_HNB$P<-Net_Removal_HNB$P_F*.99+Net_Removal_HNB$P_Bol*.97*.99+Net_Removal_HNB$P_Brh*.99+Net_Removal_HNB$P_Brk*.9*.99
...
AEC_All_HNB<-read.csv2("~/Project_Master/Test_Rep/Output/Manuscript/HN_BAS/40_WTH/Edited Data/AEC_All.csv")
...
AEC_All_HNB_R1<-AEC_All_HNB_R1 %>% subset(Date %in% c('2005-08-01')) %>% rename(P=PO4_Net) %>% 
  mutate(N=0, K=0, Ca=0, Site="HNB", Harvest="40WTH") %>% 
  select(N, P, K, Ca,Site, Harvest, Rot, Loss1, Loss2)
...
ggarrange(Ca_LNS, Ca_HNS, Ca_LNB, Ca_HNB, labels=c("A", "B", "C", "D"),
  ...
  annotate_figure(left = textGrob(expression("Ca Loss ( kg"~'⋅'~ha^{-1}~')'), rot = 90, vjust=.45, gp = gpar(cex = 1.3,
                  ...
                  bottom = textGrob("Time Frame", gp = gpar(cex = 1.3)), top=textGrob("40 WTH")) %>%
