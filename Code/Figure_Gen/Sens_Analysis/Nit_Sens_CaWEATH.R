...
LNSED_Nit_Max_Ca<-sum(Nit_10_Plus_LNSED$Ca, na.rm = TRUE)

HNSED_Nit_Max_Ca<-sum(Nit_10_Plus_HNSED$Ca, na.rm = TRUE)

LNBAS_Nit_Max_Ca<-sum(Nit_10_Plus_LNBAS$Ca, na.rm = TRUE)

HNBAS_Nit_Max_Ca<-sum(Nit_10_Plus_HNBAS$Ca, na.rm = TRUE)

LNSED_Nit_Max_K<-sum(Nit_10_Plus_LNSED$K, na.rm = TRUE)

HNSED_Nit_Max_K<-sum(Nit_10_Plus_HNSED$K, na.rm = TRUE)

LNBAS_Nit_Max_K<-sum(Nit_10_Plus_LNBAS$K, na.rm = TRUE)

HNBAS_Nit_Max_K<-sum(Nit_10_Plus_HNBAS$K, na.rm = TRUE)
...
LNSED_Nit_Min_Ca<-sum(Nit_10_Minus_LNSED$Ca, na.rm = TRUE)

HNSED_Nit_Min_Ca<-sum(Nit_10_Minus_HNSED$Ca, na.rm = TRUE)

LNBAS_Nit_Min_Ca<-sum(Nit_10_Minus_LNBAS$Ca, na.rm = TRUE)

HNBAS_Nit_Min_Ca<-sum(Nit_10_Minus_HNBAS$Ca, na.rm = TRUE)

LNSED_Nit_Min_K<-sum(Nit_10_Minus_LNSED$K, na.rm = TRUE)

HNSED_Nit_Min_K<-sum(Nit_10_Minus_HNSED$K, na.rm = TRUE)

LNBAS_Nit_Min_K<-sum(Nit_10_Minus_LNBAS$K, na.rm = TRUE)

HNBAS_Nit_Min_K<-sum(Nit_10_Minus_HNBAS$K, na.rm = TRUE)


# Percent difference is 100*(Final-Initial/initial)

PerChange_LNSED_Min_Ca<-Diff(LNSED_Nit_Min_Ca, Orig_LNSED_Ca)
PerChange_HNSED_Min_Ca<-Diff(HNSED_Nit_Min_Ca, Orig_HNSED_Ca)
PerChange_LNBAS_Min_Ca<-Diff(LNBAS_Nit_Min_Ca, Orig_LNBAS_Ca)
PerChange_HNBAS_Min_Ca<-Diff(HNBAS_Nit_Min_Ca, Orig_HNBAS_Ca)

PerChange_LNSED_Max_Ca<-Diff(LNSED_Nit_Max_Ca, Orig_LNSED_Ca)
PerChange_HNSED_Max_Ca<-Diff(HNSED_Nit_Max_Ca, Orig_HNSED_Ca)
PerChange_LNBAS_Max_Ca<-Diff(LNBAS_Nit_Max_Ca, Orig_LNBAS_Ca)
PerChange_HNBAS_Max_Ca<-Diff(HNBAS_Nit_Max_Ca, Orig_HNBAS_Ca)

PerChange_LNSED_Min_K<-Diff(LNSED_Nit_Min_K, Orig_LNSED_K)
PerChange_HNSED_Min_K<-Diff(HNSED_Nit_Min_K, Orig_HNSED_K)
PerChange_LNBAS_Min_K<-Diff(LNBAS_Nit_Min_K, Orig_LNBAS_K)
PerChange_HNBAS_Min_K<-Diff(HNBAS_Nit_Min_K, Orig_HNBAS_K)

PerChange_LNSED_Max_K<-Diff(LNSED_Nit_Max_K, Orig_LNSED_K)
PerChange_HNSED_Max_K<-Diff(HNSED_Nit_Max_K, Orig_HNSED_K)
PerChange_LNBAS_Max_K<-Diff(LNBAS_Nit_Max_K, Orig_LNBAS_K)
PerChange_HNBAS_Max_K<-Diff(HNBAS_Nit_Max_K, Orig_HNBAS_K)

LNS_Ca_Min<-Sens_Coeff(LNSED_Nit_Min_Ca,Orig_LNSED_Ca, .5, 1)
LNS_Ca_Max<-Sens_Coeff(LNSED_Nit_Max_Ca,Orig_LNSED_Ca, 1.5, 1)
LNB_Ca_Min<-Sens_Coeff(LNBAS_Nit_Min_Ca,Orig_LNBAS_Ca, .5, 1)
LNB_Ca_Max<-Sens_Coeff(LNBAS_Nit_Max_Ca,Orig_LNBAS_Ca, 1.5, 1)
HNB_Ca_Min<-Sens_Coeff(HNBAS_Nit_Min_Ca,Orig_HNBAS_Ca, .5, 1)
HNB_Ca_Max<-Sens_Coeff(HNBAS_Nit_Max_Ca,Orig_HNBAS_Ca, 1.5, 1)
HNS_Ca_Max<-Sens_Coeff(HNSED_Nit_Max_Ca,Orig_HNSED_Ca, 1.5, 1)
HNS_Ca_Min<-Sens_Coeff(HNSED_Nit_Min_Ca,Orig_HNSED_Ca, 1.5, 1)


LNS_K_Min<-Sens_Coeff(LNSED_Nit_Min_K,Orig_LNSED_K, .5, 1)
LNS_K_Max<-Sens_Coeff(LNSED_Nit_Max_K,Orig_LNSED_K, 1.5, 1)
LNB_K_Min<-Sens_Coeff(LNBAS_Nit_Min_K,Orig_LNBAS_K, .5, 1)
LNB_K_Max<-Sens_Coeff(LNBAS_Nit_Max_K,Orig_LNBAS_K, 1.5, 1)
HNB_K_Min<-Sens_Coeff(HNBAS_Nit_Min_K,Orig_HNBAS_K, .5, 1)
HNB_K_Max<-Sens_Coeff(HNBAS_Nit_Max_K,Orig_HNBAS_K, 1.5, 1)
HNS_K_Max<-Sens_Coeff(HNSED_Nit_Max_K,Orig_HNSED_K, 1.5, 1)
HNS_K_Min<-Sens_Coeff(HNSED_Nit_Min_K,Orig_HNSED_K, 1.5, 1)
...
G_NitCa<-ggplot(data = Y, aes(x = Site, y = value, fill=Inc, pattern = ID, width=.75)) +
  ...
  geom_hline(yintercept=0, linetype=1, color="black", size=.5)+ggtitle("Nitrification Increment")+
...
write.csv2(Sens_Coeff_tbl, "Sens_Tbl_Nit_Weath.csv")
write.csv2(Y, "Nit_Weath.csv")
write.csv(PerChange_Df, "NitWeath_Per.csv")