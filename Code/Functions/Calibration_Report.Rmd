...
Reliable_Table %>% mutate_if(is.numeric, format, digits=3,nsmall = 0) %>%
  kbl(caption = "Table 3: Average Soil Solution Concentrations of Reliable Months", col.names=c("Soil Layer", "Ca", "Mg", "K", "Na", "NO3", "NH4", "SO4", "Cl", "PO4", "DOC", "Al", "Si", "H+", "pH", "R", "HR"), align = "l") %>%t(.)%>% kable_styling("striped") %>%
    kable_classic(full_width = F, html_font = "Times") %>% add_header_above(c(" "=1,  "$\\\\u$mol/L"=16))
```
...
```
<br>
```{r, echo=FALSE, fig.show="holf", out.width="50%", fig.cap="Figure 10: Magnesium Weathering by Layer"}
...
```{r, echo=FALSE, fig.cap="Figure 16: SOM Carbon Per Month ", fig.show="holf", out.width="50%"}
...
Fol_Bi+labs(y=expression(Foliar~Biomass~(g/m^2)))
...
Bol_Bi+labs(y=expression(Wood~Bole~Biomass~(g/m^2)))
...
Bark_Bi+labs(y=expression(Bark~Biomass~(g/m^2)))
...
Lig_Bi+labs(y=expression(Total~Ligneous~(g/m^2)))
