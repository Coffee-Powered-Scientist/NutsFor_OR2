...
files <- list.files(path = "~/NutsforSEDLN/80_WTH/Output data/SOM", pattern = "SOM", full.names = TRUE)
SOM<- sapply(files, read.csv2, simplify=FALSE, header=FALSE) %>% 
  bind_rows(.id = "id")


SOM_Min<-read.csv2("~/NutsforSEDLN/80_WTH/Output data/SOM/OM mineralization.csv")
