...
Tree_Nut_2<-Tree_Nut %>%
  select(V1:V3, V10:V15, V16, V23:V29, V36:V42, V50:V61)

Tree_Foliage<-Tree_Nut_2 %>% select(V1:V3, V10:V15) %>% row_to_names (row_number=2) %>%
  rename(K_F='K kg/ha', Ca_F="Ca kg/ha", N_F="N kg/ha", Mg_F="Mg kg/ha", S_F="S kg/ha", P_F="P kg/ha", Dry.weight="Dry weight")

Tree_Foliage[c(1:9)]<-lapply(Tree_Foliage[c(1:9)], as.numeric)

Tree_Wood<-Tree_Nut_2 %>% select(V16:V28) %>% row_to_names (row_number=2) %>%
  rename(K_Bol='K kg/ha', Ca_Bol="Ca kg/ha", N_Bol="N kg/ha", Mg_Bol="Mg kg/ha", S_Bol="S kg/ha", P_Bol="P kg/ha")
Tree_Wood[c(1:7)]<-lapply(Tree_Wood[c(1:7)], as.numeric)

Tree_Bark<-Tree_Nut_2 %>% select(V29:V41) %>% row_to_names (row_number=2) %>%
  rename(K_Brk='K kg/ha', Ca_Brk="Ca kg/ha", N_Brk="N kg/ha", Mg_Brk="Mg kg/ha", S_Brk="S kg/ha", P_Brk="P kg/ha")
Tree_Bark[c(1:7)]<-lapply(Tree_Bark[c(1:7)], as.numeric)

Tree_Branch<-Tree_Nut_2 %>% select(V42:V54) %>% row_to_names (row_number=2) %>%
  rename(K_Brh='K kg/ha', Ca_Brh="Ca kg/ha", Mg_Brh="Mg kg/ha", S_Brh="S kg/ha", P_Brh="P kg/ha")
...
write.csv2(Tree_Nutrients_All)

Tree_Nut_Ca<-Tree_Nutrients_All %>% select(Ca_F, Ca_Bol, Ca_Brh, Ca_Brk, YEAR, Month)
...
Ca_Test<-ggplot(Tree_Nut_Ca_Melt, aes(x=Date, y=value, fill=variable))+geom_area()
