...
setwd("~/Project_Master/Test_Rep/Output/Manuscript/LN_SED/40_BO/Sens/Growth/Lower")
library(dplyr)
library(tidyverse)
library(lubridate)

#Reads all files in folder at once, then 
files <- list.files(path = "~/Sens_Parent/Sens_Growth/LN_SED/Minimum/Output data/Soil solution", pattern = "*.csv", full.names = TRUE)
