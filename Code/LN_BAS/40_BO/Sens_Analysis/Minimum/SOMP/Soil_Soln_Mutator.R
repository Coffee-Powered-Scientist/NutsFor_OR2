...
setwd("~/Project_Master/Test_Rep/Output/Manuscript/LN_BAS/40_BO/Sens/SOMP/Lower")
library(dplyr)
library(tidyverse)
library(lubridate)

#Reads all files in folder at once, then 
files <- list.files(path = "~/Sens_Parent/Sens_SOMP/LN_BAS/Minimum/Output data/Soil solution", pattern = "*.csv", full.names = TRUE)
