...
files <- list.files(path = "~/NutsforBASLN/80_BO/Output data/SOM", pattern = "SOM", full.names = TRUE)
SOM<- sapply(files, read.csv2, simplify=FALSE, header=FALSE) %>% 
  bind_rows(.id = "id")

SOM_Min<-read.csv2("~/NutsforBASLN/80_BO/Output data/SOM/OM mineralization.csv")
