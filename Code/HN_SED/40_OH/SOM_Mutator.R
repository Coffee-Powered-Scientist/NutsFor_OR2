...
setwd("~/Project_Master/Test_Rep/Output/Manuscript/HN_SED/40_OH/Edited Data")
...
files <- list.files(path = "~/NutsforSEDHN/40_OH/Output data/SOM", pattern = "SOM", full.names = TRUE)
SOM<- sapply(files, read.csv2, simplify=FALSE, header=FALSE) %>% 
  bind_rows(.id = "id")

SOM_Min<-read.csv2("~/NutsforSEDHN/40_OH/Output data/SOM/OM mineralization.csv")
...
SOM_Mut<- SOM %>%
  ...
  select(C, N, Ca, Mg, K, S, P ,Date, MB_N)
