...
setwd("~/Project_Master/Test_Rep/Output/Manuscript/HN_SED/40_BO/Sens/Nit/Lower")
library(dplyr)
library(tidyverse)
library(lubridate)

#Reads all files in folder at once, then 
files <- list.files(path = "~/Sens_Parent/Sens_Nit/HN_SED/40_BO/Minimum/Output data/Soil solution", pattern = "*.csv", full.names = TRUE)
