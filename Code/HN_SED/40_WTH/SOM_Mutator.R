...
files <- list.files(path = "~/NutsforSEDHN/40_WTH/Output data/SOM", pattern = "SOM", full.names = TRUE)
SOM<- sapply(files, read.csv2, simplify=FALSE, header=FALSE) %>% 
  bind_rows(.id = "id")

SOM_Min<-read.csv2("~/NutsforSEDHN/40_WTH/Output data/SOM/OM mineralization.csv")
